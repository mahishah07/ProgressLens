...
describe("PMS white-box branch and path tests", () => {
	...
	test("buildDashboard executes null-score and 0/1/many loop paths", async () => {
		Assessment.find.mockReturnValueOnce({ sort: jest.fn().mockResolvedValue([]) });
		expect((await progressService.buildDashboard("DAS-0707")).status).toBe("assessment_pending");
		...
		Assessment.find.mockReturnValueOnce({ sort: jest.fn().mockResolvedValue([first]) });
		...
		const second = assessment("a2", "2026-07-01", { newBand: "B5", wraScore: 9 });
		Assessment.find.mockReturnValueOnce({ sort: jest.fn().mockResolvedValue([first, second]) });
		const many = await progressService.buildDashboard("DAS-0707");
		expect(many.componentTrend).toHaveLength(2);
		expect(many.assessmentHistory.map((item) => item.semester)).toEqual(["a2", "a1"]);
	});
	...
	test.each([
		...
		["B5", "B4", "declined", 1],
		["B4", "B4", "same", 0],
	])("compareAssessments branch %s→%s is %s", async (from, to, direction, steps) => {
	...
	test("compareAssessments rejects an invalid explicit selection path", async () => {
		...
		const result = await comparisonService.compareAssessments("DAS-0707", "a1", "missing");
		expect(result).toMatchObject({ status: "insufficient_data", message: "Invalid assessment selection" });
	});
	...
	test("generateReport covers unknown, insufficient, and successful persistence paths", async () => {
		...
		Assessment.find.mockReturnValueOnce({ sort: jest.fn().mockResolvedValue([]) });
		expect(await reportService.generateReport("DAS-0707")).toMatchObject({ status: "insufficient_data" });
		...
		const success = await reportService.generateReport("DAS-0707", "teacher-01");
		expect(success.status).toBe("ok");
		expect(Report.create).toHaveBeenCalledWith(expect.objectContaining({ generatedBy: "teacher-01", isEdited: false }));
	});
