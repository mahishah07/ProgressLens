...
async function postStudent(id, overrides) {
	const response = await request(app).post("/api/students").send(studentPayload(id, overrides));
...
describe("PMS end-to-end workflows", () => {
	...
	test("E2E-008: student overview, history, and dashboard remain consistent with MongoDB", async () => {
		...
		await postAssessment(student.studentId, "2026 Sem 1", "2026-01-15", { teacherComments: "Baseline" });
		...
		expect([overview.status, history.status, dashboard.status]).toEqual([200, 200, 200]);
		expect(overview.body.lastSemester).toBe("2026 Sem 2");
		expect(history.body.map((item) => item.semester)).toEqual(["2026 Sem 1", "2026 Sem 2"]);
		expect(dashboard.body.latestAssessment.semester).toBe("2026 Sem 2");
		expect(dashboard.body.totalAssessments).toBe(await Assessment.countDocuments({ student: student._id }));
	});

	test("E2E-009: comparison matches independently known band and component changes", async () => {
		const student = await postStudent("DAS-0707");
		await postAssessment(student.studentId, "2026 Sem 1", "2026-01-15", { newBand: "B4" });
		...
		expect(response.body.bandChange).toEqual({ direction: "improved", steps: 1 });
		expect(response.body.componentComparison.wra.change).toBe(3);
		expect(response.body.comparisonPeriod).toEqual({ from: "2026 Sem 1", to: "2026 Sem 2" });
	});
	...
	test("E2E-010: generate, edit, and reload a parent report without changing protected fields", async () => {
		...
		await postAssessment(student.studentId, "2026 Sem 2", "2026-07-15", { newBand: "B5" });
		...
		const edited = await request(app)
			...
			.send({ teacherObservations: "Practise phonics daily.", generatedBy: "intruder" });
		...
		expect(JSON.stringify(latest.body)).not.toMatch(/diagnos|secret|api[_-]?key/i);
	});
	...
	test("E2E-011: recommendation is saved only to the correct latest assessment", async () => {
		...
		aiService.generateRecommendations.mockResolvedValue({ summary: "Use guided reading." });

		const response = await request(app).post("/api/ai/DAS-0707/recommendations");
		expect(response.status).toBe(200);
		const firstLatest = await Assessment.findOne({ student: first._id }).sort({ assessmentDate: -1 });
		const secondLatest = await Assessment.findOne({ student: second._id }).sort({ assessmentDate: -1 });
	...
	test("E2E-015: parallel histories for two students never cross records", async () => {
		...
		await Promise.all([
			postAssessment(first.studentId, "2026 Sem 1", "2026-01-15", { teacherComments: "FIRST-ONLY" }),
			postAssessment(second.studentId, "2026 Sem 1", "2026-01-15", { teacherComments: "SECOND-ONLY" }),
		]);
